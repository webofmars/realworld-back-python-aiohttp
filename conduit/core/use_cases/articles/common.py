...
async def get_articles(unit_of_work: UnitOfWork, filter: ArticleFilter, *, limit: int, offset: int) -> list[Article]:
    ...
    LOG.info("got articles", extra={"filter": filter, "article_ids": [article.id for article in articles]})
    return articles
...
async def get_article_count(unit_of_work: UnitOfWork, filter: ArticleFilter) -> int:
    ...
    LOG.info("got article count", extra={"filter": filter, "count": count})
    return count
...
async def get_author(unit_of_work: UnitOfWork, author_id: UserId) -> User:
    ...
    LOG.info("got article author", extra={"author_id": author_id})
    return author
...
async def get_tags_for_article(unit_of_work: UnitOfWork, article_id: ArticleId) -> list[Tag]:
    ...
    LOG.info("got article tags", extra={"article_id": article_id, "tags": tags})
    return tags
...
async def get_tags_for_articles(
    ...
    LOG.info("got articles tags", extra={"article_ids": article_ids})
    return tags
...
async def is_favorite(unit_of_work: UnitOfWork, article_id: ArticleId, of: UserId | None) -> bool:
    ...
    LOG.info(
        "got article favorite status",
        extra={"user_id": of, "article_id": article_id, "is_favorite": is_favorite},
    )
    return is_favorite
...
async def are_favorite(
    ...
    LOG.info(
        "got articles favorite status",
        extra={"user_id": of, "article_ids": article_ids, "are_favorite": are_favorite},
    )
    return are_favorite
...
async def get_favorite_count_for_article(unit_of_work: UnitOfWork, article_id: ArticleId) -> int:
    ...
    LOG.info("get favorite count for article", extra={"article_id": article_id, "count": count})
    return count
...
async def get_favorite_count_for_articles(
    ...
    LOG.info("get favorite count for articles", extra={"article_ids": article_ids, "count": count})
    return count
